...
class Player {
  ...
  takeItem(item) {
    if (this._pack.length === 3) {
      console.log(`Pack is full so ${item.name} could not be stored`);
      return false;
    }
  ...
  discardItem(item) {
    ...
    if (index === -1) {
      console.log(`Pack does not contain ${item.name}. Discard failed.`);
      return false;
    }
  ...
  equippedWith(){
    if (!this.equipped){
      console.log(`${this.name} already has an item equipped`);
      return false;
    }
